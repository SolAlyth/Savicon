...
export const get_video_element = () => {
    const video = document.getElementsByTagName("video")[0];
    return (video !== undefined) ? video : null;
};
export const play_toggle = (video) => {
    if (video.paused)
        video.play();
    else
        video.pause();
};
export const absolute_jump = (video, playtime) => {
    video.currentTime = playtime.to_secs();
};
export const relative_jump = (video, time) => {
    video.currentTime += time;
};
export const speed_faster = (video) => {
    if (video.playbackRate <= SPEED_MAX - SPEED_INTERVAL)
        video.playbackRate += SPEED_INTERVAL;
};
export const speed_slower = (video) => {
    if (SPEED_MIN + SPEED_INTERVAL <= video.playbackRate)
        video.playbackRate -= SPEED_INTERVAL;
};
const save_playtime = (video, id) => {
    document.cookie = `${id}=${Math.floor(video.currentTime)}; Max-age=${86400 * COOKIE_AGE_DAYS}`;
};
export const create_save_cycle = (video, id) => {
    setInterval(() => { save_playtime(video, id); }, 1000 * INTERVAL_DELAY_SECS);
};
export const load_playtime = (video, id, confirm) => {
...
export const input_playtime = (input_message, error_message) => {
