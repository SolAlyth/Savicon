import { Key, KeyMap, hostname_check, get_lesson_id, is_running, running, get_video_element, play_toggle, absolute_jump, relative_jump, speed_faster, speed_slower, create_save_cycle, load_playtime, input_playtime } from "./lib.js";
export const main = (window) => {
    ...
    const keymap = new KeyMap([new Key("Space"), () => play_toggle(video)], [new Key("ArrowLeft"), () => relative_jump(video, -5)], [new Key("ArrowRight"), () => relative_jump(video, 5)], [new Key("Comma"), () => speed_slower(video)], [new Key("Period"), () => speed_faster(video)], [new Key("KeyJ"), () => {
